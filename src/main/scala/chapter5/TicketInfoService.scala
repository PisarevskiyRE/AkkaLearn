...
trait TicketInfoService extends WebServiceCalls {
  ...
  def getWeather(ticketInfo: TicketInfo): Future[TicketInfo] = {
    ...
    val futureWeatherX: Future[Option[Weather]] = callWeatherXService(ticketInfo).recover(withNone)

    val futureWeatherY: Future[Option[Weather]] = callWeatherYService(ticketInfo).recover(withNone)
